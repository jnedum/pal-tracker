...
public class JdbcTimeEntryRepositoryTest {
    ...
    public void createInsertsATimeEntryRecord() throws Exception {
        ...
        Map<String,Object> foundEntry = jdbcTemplate.queryForMap("select * from time_entries where id=?",
                entry.getId());

        assertEquals(entry.getId(), foundEntry.get("id"));
        assertEquals(123L, foundEntry.get("project_id"));
        assertEquals(321L, foundEntry.get("user_id"));
        assertEquals("today", foundEntry.get("date"));
        assertEquals(8, foundEntry.get("hours"));
    }
